...
const mountNode = document.getElementById('liquidEtherLayer');

if (mountNode) {
  const liquidEtherRoot = createRoot(mountNode);
  liquidEtherRoot.render(
  ...
  window.addEventListener('beforeunload', () => liquidEtherRoot.unmount(), { once: true });
}
